...
//! Run with:
//!   PHASE_CASE_INDEX=12 cargo test -p d2-lib --test phase_probe probe_case -- --ignored --nocapture
...
fn parent_chain_has_cycle(g: &d2_graph::Graph, start: usize) -> bool {
...
fn probe_case() {
    ...
    let (ast_map, parse_err) = d2_parser::parse("", &case.script);
    assert!(parse_err.is_none(), "parse failed: {parse_err:?}");
    eprintln!("parse ok");

    let ir_map = d2_ir::compile(&ast_map).expect("ir");
    eprintln!(
        "ir ok: fields={}, edges={}",
        ir_map.fields.len(),
        ir_map.edges.len()
    ...
    let mut g = d2_compiler::compile("", &case.script).expect("compiler");
    eprintln!(
    ...
    if let Some(theme) = d2_themes::catalog::find(case.theme_id) {
        g.theme = Some(theme.clone());
    }

    let mut ruler = d2_textmeasure::Ruler::new().expect("ruler");
    d2_lib::set_dimensions(&mut g, &mut ruler).expect("set_dimensions");
    eprintln!("set_dimensions ok");

    d2_dagre_layout::layout(&mut g, None).expect("layout");
    eprintln!("layout ok");

    if std::env::var("PHASE_DUMP_DIAGRAM").as_deref() == Ok("1") {
        let diagram = d2_exporter::export(&g, None, None).expect("export");
    ...
    let svg = d2_lib::d2_to_svg(&case.script).expect("d2_to_svg");
