//! wasm-bindgen wrapper around `d2-lib`.
//!
...
//! into an SVG string. d2-lib uses its own pure-Rust dagre layout, so
//! no external JS bridge is required — unlike the plantuml-little wasm
//! wrapper, which has to be wired up to a Graphviz engine.
//!
...
///
/// Errors from the underlying `d2-lib` converter are surfaced as a
/// JavaScript `Error` with the Rust error message.
#[wasm_bindgen]
pub fn convert(input: &str) -> Result<String, JsValue> {
    d2_lib::d2_to_svg(input)
        .map(|bytes| String::from_utf8_lossy(&bytes).into_owned())
        .map_err(|e| JsValue::from_str(&e))
}

/// Version of the compiled `d2-lib-web` wasm.
#[wasm_bindgen]
